	...
	var b = {
		init: function(b) {
			...
			function m(a, b, c) {
				...
				if (c = c || s, b = i(b), b === !1 || 0 === (d = b.value.length)) return m.empty().hide(), a;
				if (c.showHeader) {
					g = "<thead><tr>";
				...
				for (p.push("<tbody>"), e = 0; d > e; e++) {
					o = 0, h = "", j = b.value[e][c.idField] || "", l = b.value[e][c.keyField] || "";
					
					
						for (f in b.value[e]) l || c.indexKey !== o || (l = b.value[e][f]), j || c.indexId !== o || (j = b.value[e][f]), o++, k(f) !== !1 && (h += '<td data-name="' + f + '">' + b.value[e][f] + "</td>");
						
					
					h = '<tr data-index="' + e + '" data-id="' + j + '" data-key="' + l + '" >' + h + "</tr>", p.push(h)
				}
				return p.push("</tbody></table>"), m.html(p.join("")).show(), n(a, m, c), m.css("max-height") && Number(m.css("max-height").replace("px", "")) < Number(m.find("table:eq(0)").css("height").replace("px", "")) && Number(m.css("min-width").replace("px", "")) < Number(m.css("width").replace("px", "")) ? m.css("padding-right", "20px").find("table:eq(0)").css("margin-bottom", "20px") : m.css("padding-right", 0).find("table:eq(0)").css("margin-bottom", 0), a
			}
			...
			return a.isFunction(s.processData) && (i = s.processData), a.isFunction(s.getData) && (h = s.getData), !b.showHeader && s.effectiveFields && s.effectiveFields.length > 1 && (s.showHeader = !0), a("head:eq(0)").append("<style>." + s.listHoverCSS + "{" + s.listHoverStyle + "}</style>"), r.each(function() {
				var i = a(this),
				...
				}) : j.css("width", "auto"), i.on("keydown", function(b) {
					var c, d = "";
